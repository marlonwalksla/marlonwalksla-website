...
window.initMapEngine = async function() {
  ...
  function renderItinerary() {
    ...
    // Map pin filtering for active day
  ...
  geojsonData.features.forEach((feature, index) => {
    ...
    const catDetails = window.MarlonSpotCard.getCategoryDetails(rawCategory, customColor, categoryMap, defaultPinSvg);
  ...
  if (filterControlsView) {
    ...
    Array.from(categories).sort().forEach(cat => {
      ...
      const catDetails = window.MarlonSpotCard.getCategoryDetails(cat, '', categoryMap, defaultPinSvg);
